...
class DeviceAtlas
  ...
  def getApiRevision()
    _getRevisionFromKeyword('$Rev: 13957 $')
  end
  ...
  def _seekProperties(node, string, properties, sought, matched, rules)
    unmatched = string
    ...
    if node.has_key?('c')
      if(node.has_key?('r'))
        ...
        rules_count = node['r'].length-1
        (0..rules_count).each do |counter|
          rule_id = node['r'][counter];
          #STDERR.puts "remaining UA portion went from\n<strong>#{string}</strong> to\n";
          #STDERR.puts "from: #{string}"
          new_string = string.gsub(/#{rules[rule_id]}/, '');
          #STDERR.puts "to: #{new_string}"
      ...
      (0..string.length+1).each do |c|
        ...
        if node['c'].kind_of?(Hash) && node['c'][seek]
          ...
          return _seekProperties(node['c'][seek], string[c+1..string.length], properties, sought, matched, rules)
